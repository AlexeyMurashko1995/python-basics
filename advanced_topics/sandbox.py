...
async def get_ai_description(product_id: int, session: Session = Depends(get_session), role_checker = Depends(RoleChecker(allowed_roles=['admin', 'manager']))):
    await asyncio.sleep(3)
    query = select(Product).where(product_id == Product.id)
    target_product = session.exec(query).first()
    if not target_product:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail='Product not found')
    ai_text = f'This is a stunning marketing description for {target_product.title}, automatically generated by AI.'
